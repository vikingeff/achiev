...
<?php include('includes/modif.php'); ?>
	...
	<div class="exmission" />
	<?php
		$reponse = mysqli_query($con,"SELECT fname_student, lname_student, domaine_student, school_student FROM student ORDER BY id_student DESC");
		$i = 0;
		while ($donnees = mysqli_fetch_array($reponse) AND $i < 5)
		{
		?>
		<h3><?php echo $donnees['fname_student']." ".$donnees['lname_student'];?></h3>
		<span class="owner">
		<?php
			$rep = mysqli_query($con, "select * from subject");
			$found = 0;
	...
	<div class="exmission" />
	<?php
		$reponse = mysqli_query($con, "SELECT fname_student, lname_student, domaine_student, school_student FROM student WHERE done_student > 0");
		$i = 0;
		while ($donnees = mysqli_fetch_array($reponse) AND $i < 5)
		{
		?>
